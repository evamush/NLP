...
trigram = lm.NGramLM(oov_train, 3)
fourthgram = lm.NGramLM(oov_train, 4)
for alpha1 in np.arange(0.1,1,0.1):
	my_LM1 = lm.InterpolatedLM(bigram,unigram,alpha1)
	
	for alpha2 in np.arange(0.1,1,0.1):
		for alpha3 in np.arange(0.1,1,0.1):
			my_LM = InterpolatedLM2 (fourthgram, trigram, my_LM1, alpha2, alpha3)
			_snlp_lm = create_lm(_snlp_vocab)
			print(alpha1,alpha2,alpha3, lm.perplexity(_snlp_lm, _snlp_test_song_words))
